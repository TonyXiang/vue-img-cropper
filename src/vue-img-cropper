<template>
    <div class="img-page-body">
        <canvas ref="canvas" id="canvas" :height="height" :width="width"></canvas>
        <label ref="chooseImgBtn" for="fileInput">
            <slot></slot>
        </label>
        <form ref="fileForm" hidden>
            <input ref="fileInput" id="fileInput" type="file" accept="image/*" hidden="hidden" @change="changeFun">
        </form>

        <div v-show="show" class="img-page">
            <div class="page-body" :style="{height: windowHeight + 'px'}">

                <img ref="img" id="img" :width="startWidth" :height="startHeight"
                     :style="{
                         top: imgTop + 'px',
                         left: imgLeft + 'px',
                ...
                <div class="img-mask top" :style="{height: maskHeight + 'px'}"></div>
                <div class="img-mask bottom" :style="{height: maskHeight + 'px'}"></div>
                <div class="img-mask left" :style="{width: maskWidth + 'px'}"></div>
                <div class="img-mask right" :style="{width: maskWidth + 'px'}"></div>

                <div id="box" class="cut-box" @wheel="mouseWheel" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend"
                     :style="{
                       height: boxHeight + 'px',
                       width: boxWidth + 'px',
                       top: boxTop + 'px',
            ...
            <div class="page-footer" :style="{height: footerHeight + 'px', 'line-height':footerHeight + 'px',}">
                <div class="btn" @click="hidePage">取消</div>
                <div class="btn" @click="cutImg">选取</div>
            </div>
        </div>
    </div>
    ...
    export default{
        ...
        methods: {
                    ...
                    EXIF.getData(file, function(){
                        ...
                        firstImg.onload = function(){
                            ...
                            image.onload = function(){
                                ...
                                // 设置图片初始高度、宽度、top、left, 裁剪框的尺寸和位置、遮罩的大小
                                if(self.windowHeight/self.windowWidth >= self.height/self.width){
                                    ...
                                    self.maskHeight = 0;
                                    self.maskWidth = (self.windowWidth-self.boxWidth)/2;
                                }
                                ...
                                if (imgCanvas) {
                                    let context2 = imgCanvas.getContext('2d');
                                    context2.drawImage(self.$refs.img, 0, 0, self.naturalWidth, self.naturalWidth * self.windowHeight/self.windowWidth, 0, 0, self.windowWidth, self.windowHeight)
                                    return
                                }
            ...
            cutImg: function(){
                ...
                if (this.startWidth * this.scale < this.boxWidth) {
                    canvasStartX = (this.boxWidth - this.startWidth * this.scale)/2 * this.width / this.boxWidth
                }

                if (this.startHeight * this.scale < this.boxHeight) {
                    canvasStartY = (this.boxHeight - this.startHeight * this.scale)/2 * this.height / this.boxHeight
                }

                ctx.drawImage(img, imgX, imgY, dWidth , dHeight, canvasStartX, canvasStartY, this.width, this.height);
                ctx.save();

                this.$emit('cutImg', canvas.toDataURL(this.fileType, this.compressionRatio))
                this.cleanInput();
                this.show = false;
        ...
        created(){
            this.footerHeight = window.innerWidth/6.4*.88;
            this.windowHeight = window.innerHeight - this.footerHeight;
